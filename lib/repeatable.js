  ...
  function getNextMillis(millis, opts) {
    ...
    const currentDate =
      opts.startDate ? new Date(opts.startDate)
        : new Date(millis);
    console.log('currentDate', new Date(opts.startDate), currentDate);
    let nextInterval = '';
    // Temporary fix to make Bull scheduler use rrule instead of cron for Recurring jobs
    // This change only applies if there is rrule object as part of request
    if (opts.rrule) {
      const rule = new RRule({ ...opts.rrule, dtstart: currentDate, count: 2 });
      nextInterval = moment(rule.all()[1]).format('x');
      console.log('Inside Rrule', nextInterval, { ...opts.rrule, dtstart: currentDate, count: 2 });
    } else {
      ...
      nextInterval = interval.next().getTime();
      console.log('Inside cron pattern', nextInterval);
    }
    
    console.log('Outside Rrule', nextInterval);

    try {
      return nextInterval;
    } catch (e) {
      console.log('Error with nextInterval', nextInterval);
      // Ignore error
    }
  }
